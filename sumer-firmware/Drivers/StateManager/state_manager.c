...
uint8_t volatile state_manager_local_name_val[11] = {0};

uint8_t * state_manager_server_local_name(void)
{

	if(state_manager_local_name_val[0]==0x00)
	{
	...
	return (uint8_t *)&state_manager_local_name_val;
}

uint8_t volatile serial_number[6]={0};

uint8_t * state_manager_server_serial_number(void)
{
	if(serial_number[0]==0x00)
	{
		serial_number[0] = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_SERIAL_NUMBER);
		serial_number[1] = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_SERIAL_NUMBER+1);
		serial_number[2] = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_SERIAL_NUMBER+2);
		serial_number[3] = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_SERIAL_NUMBER+3);
		serial_number[4] = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_SERIAL_NUMBER+4);
		serial_number[5] = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_SERIAL_NUMBER+5);
	}
...
void state_manager_initialize(void)
{
	...
	is_seismic_log_enabled = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_IS_SEISMIC_LOG_ENABLED);
	activity_threshold_low = local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_ACTIVITY_THRESHOLD_VAL);
	activity_time=	local_settings_get_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_ACTIVITY_TIME_VAL);
...
void state_manager_commit_to_flash(void) {
	...
	{
		local_settings_set_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_IS_SEISMIC_LOG_ENABLED,is_seismic_log_enabled);
		state_of_is_sesimic_log_enabled = STATE_MANAGER_FIELD_STATUS_SYCNED;
	}
	if (state_of_activity_threshold_low == STATE_MANAGER_FIELD_STATUS_WAITING_TO_COMMIT)
	{
		local_settings_set_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_ACTIVITY_THRESHOLD_VAL,activity_threshold_low);
		state_of_activity_threshold_low = STATE_MANAGER_FIELD_STATUS_SYCNED;
	}
	if (state_of_activity_time == STATE_MANAGER_FIELD_STATUS_WAITING_TO_COMMIT)
	{
		local_settings_set_char_value(FLASH_STORAGE_FLASH_CHIP_ADDR_ACTIVITY_TIME_VAL,activity_time);
		state_of_activity_time = STATE_MANAGER_FIELD_STATUS_SYCNED;
	}
}
